    ...
    <form onSubmit={submit} className="space-y-6">
      <div className={cardCls("space-y-4")}>
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="space-y-2"><Label>Total amount (ZAR)</Label><Input type="number" inputMode="decimal" step="0.01" min="0" value={totalAmount} onChange={(e) => setTotalAmount(Number(e.target.value))} required /></div>
        </div>
      ...
      <div className={cardCls("space-y-3")}>
        <div className="flex items-center justify-between">
          ...
          <div className={`text-sm tabular-nums ${Math.abs(contribTotal - totalAmount) < 0.01 ? "text-emerald-600" : "text-destructive"}`}>
            {formatZAR(contribTotal)} / {formatZAR(totalAmount)}
          </div>
        </div>
